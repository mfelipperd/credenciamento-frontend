...
export function ExpensesByCategoryChart({ data, loading }: Props) {
  ...
  if (!hasData) return <EmptyChart label="Despesas por Categoria" loading={loading} height={320} />;

  const options: ApexOptions = {
    ...darkBase,
    chart: { ...darkBase.chart, type: "donut" },
    labels: data.labels,
    colors: data.colors?.length ? data.colors : ["#EB2970", "#00aacd", "#FEB019", "#775DD0", "#3F51B5"],
